...
function GrabVoxelShape(delta_t)
  ...
  if GrabShape then
    ...
    local force = 
      Math.vec_scale(Math.pow(Math.min(dist_to_grab_pos, scale_dist) / scale_dist, scale_power) * force_factor, to_grab_pos)
...
function PushHint(hint, duration, start_function)
  ...
  table.insert(Hints, #Hints + 1, {hint=hint, alpha=0.0, init=false, time_alive=0.0, duration=duration, start_function=start_function})
end
...
function InitTutorial()
  InteractorStates = {fuse_box=true}
end
...
function OnActivate(entity)
  ...
  PushHint("\"Whoa, where am I? Did I get kidnapped or something? This is not good. I need to get out of here, and fast.\"", 4.0)
  PushHint("\"Ugh, it's freezing in here. I can barely see a thing. There's got to be a light switch or something. I can't believe I'm stuck in here like a rat in a trap.\"", 4.0, nil)
  PushHint("Press and hold [LMB] to lift objects. Use in combination with [E]/[Q] to pull objects up/down.", 0.0, InitTutorial)
